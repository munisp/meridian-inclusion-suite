...
export default function Band() {
  ...
  return (
    <div className="space-y-4">
      <div className="card">
        <h2 className="font-bold text-neutral-800">My presumptive band</h2>
        {profile ? (
          <p className="text-sm text-stone-600 mt-1">
            {profile.state} · {profile.tradeCategory.replace('_', ' ')} · turnover {naira(profile.annualTurnoverKobo)}
          </p>
        ) : (
          <p className="text-sm text-warning-on mt-1">Save your profile first.</p>
        )}
        <button className="btn-primary w-full mt-3" disabled={busy || !profile} onClick={evaluate}>
          {busy ? 'Evaluating…' : 'Evaluate my band'}
        </button>
        {error && <p role="alert" className="text-sm text-danger-strong mt-2">{error}</p>}
      </div>
      {result && (
        <div className="card space-y-2">
          {result.exempt ? (
            <>
              <p className="text-lg font-bold text-success-on">Exempt</p>
            ...
            <>
              <p className="text-lg font-bold text-neutral-800">{result.band_label}</p>
              <p className="text-2xl font-bold">{naira(result.annual_levy_kobo)}<span className="text-sm font-normal text-stone-600"> / year</span></p>
              <p className="text-sm text-stone-600">
                or {naira(result.monthly_levy_kobo)} / month · admin fee {naira(result.admin_fee_kobo)}
              </p>
              <p className="text-xs text-stone-600">Rule pack: {result.pack_id}@{result.pack_version}</p>
              <details className="text-xs text-stone-600">
                <summary className="cursor-pointer font-medium text-brand-700">Calculation trace</summary>
