import { useState } from 'react'
import { educationApi, naira, EDUCATION_URL } from '../api'

interface CalcResult {
  amount_kobo: number
  amount_naira: number
  provision_citation: string
  disclaimer: string
  trace: string[]
}

export default function Education() {
  const [income, setIncome] = useState('3000000')
  const [rent, setRent] = useState('0')
  const [pit, setPit] = useState<CalcResult | null>(null)
  const [faqQ, setFaqQ] = useState('')
  const [faqHits, setFaqHits] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)

  async function runPit() {
    setError(null)
    try {
      const resp = await educationApi.post<CalcResult>('/v1/calc/pit', {
        annual_gross_income_kobo: Math.round(parseFloat(income || '0') * 100),
        annual_rent_paid_kobo: Math.round(parseFloat(rent || '0') * 100),
      })
      setPit(resp.data)
    } catch (e: any) {
      setError('Education service unreachable: ' + (e?.message || e))
    }
  }

  async function searchFaq() {
    if (faqQ.trim().length < 2) return
    try {
      const resp = await educationApi.get('/v1/faq/search', { params: { q: faqQ, limit: 3 } })
      setFaqHits(resp.data.results)
    } catch {
      setFaqHits([])
    }
  }

  return (
    <div className="space-y-4">
      <div className="card">
        <h2 className="font-bold text-sand-800 mb-2">PIT calculator (NTA 2025)</h2>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="label">Annual income (₦)</label>
            <input className="input" inputMode="numeric" value={income} onChange={(e) => setIncome(e.target.value)} />
          </div>
          <div>
            <label className="label">Annual rent paid (₦)</label>
            <input className="input" inputMode="numeric" value={rent} onChange={(e) => setRent(e.target.value)} />
          </div>
        </div>
        <button className="btn-primary w-full mt-3" onClick={runPit}>Calculate</button>
        {error && <p className="text-xs text-red-700 mt-2">{error}</p>}
        {pit && (
          <div className="mt-3 space-y-1">
            <p className="text-xl font-bold">{naira(pit.amount_kobo)}</p>
            <p className="text-xs text-stone-500">{pit.provision_citation}</p>
            <details className="text-xs text-stone-600">
              <summary className="cursor-pointer font-medium text-sand-700">Trace</summary>
              <ul className="mt-1 space-y-1 list-disc pl-4">{pit.trace.map((t, i) => <li key={i}>{t}</li>)}</ul>
            </details>
            <p className="text-[11px] text-stone-400">{pit.disclaimer}</p>
          </div>
        )}
      </div>
      <div className="card">
        <h2 className="font-bold text-sand-800 mb-2">Tax FAQ search</h2>
        <div className="flex gap-2">
          <input className="input" placeholder="e.g. WHT rates" value={faqQ} onChange={(e) => setFaqQ(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && searchFaq()} />
          <button className="btn-ghost" onClick={searchFaq}>Search</button>
        </div>
        <ul className="mt-2 space-y-2">
          {faqHits.map((h) => (
            <li key={h.id} className="text-sm">
              <p className="font-medium">{h.question}</p>
              <p className="text-xs text-stone-600 mt-0.5">{h.answer}</p>
            </li>
          ))}
        </ul>
      </div>
      <div className="card">
        <h2 className="font-bold text-sand-800 mb-2">Ask the tax assistant (T14)</h2>
        <iframe
          title="NRS Tax Help"
          src={`${EDUCATION_URL}/embed/chat`}
          className="w-full h-[520px] rounded-lg border border-sand-200 bg-sand-50"
          loading="lazy"
        />
      </div>
    </div>
  )
}
